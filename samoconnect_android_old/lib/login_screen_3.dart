...
//      print(scrollPercent);
  ...
  loginPressed(String email, String password){
    print("EMALLL");
    print(email);
    print("PASWORRD");
    print(password);
    ...
    if(email!=""){
      if(password!=""){
        FirebaseAuth.instance.signInWithEmailAndPassword(
          ...
          (onError){
            print("ERORORORR");
            String error = onError.toString().split("(")[1].split(",").first;
            print(onError.toString().split("(")[1].split(",").first);
            if(error == "ERROR_INVALID_EMAIL"){
              loginKey.currentState.showSnackBar(unformattedEmailSnackbar);

            }else if(error == "ERROR_USER_NOT_FOUND"){
              loginKey.currentState.showSnackBar(noAccountSnackbar);
            }else if(error == "ERROR_WRONG_PASSWORD"){
              loginKey.currentState.showSnackBar(invalidPasswordSnackbar);
            }else{
              loginKey.currentState.showSnackBar(networkSnack);
        ...
        FirebaseAuth.instance.signInWithEmailAndPassword(
          ...
          FirebaseAuth.instance.currentUser().then((onValue){
            print("HMMMM");
            print(onValue);
            if(onValue!=null&&!onValue.isAnonymous){
              Navigator.of(context).pushNamed(MainWindowAndroid.tag);
            }
          });
      }else{
